...
async function info(query) {
  try {
    ...
    // Get best artifacts
    $('.character-build-section').eq(1).find('.character-build-weapon').each((i, el) => {
      const artifact = {
        rank: $(el).find('.character-build-weapon-rank').text(),
        name: $(el).find('.character-build-weapon-name').text(),
        icon: $(el).find('.character-build-weapon-icon').attr('src'),
    ...
    $('.character-stats-item').each((i, el) => {
      const key = $(el).html().split(':')[0].replace(/<\/?b>/g, '');
    ...
    $('.character-team').each((i, el) => {
      ...
      $(el).find('.character-portrait').each((j, charEl) => {
        const character = {
...
async function character() {
    try {
        ...
        $('.character-portrait').each((index, element) => {
            ...
            const rarity = $(element).find('.character-icon').attr('class').includes('rarity-5') ? 5 : 4;
...
async function build() {
  try {
    ...
    $('.builds-list-item').each((i, element) => {
      ...
      const weaponName = $(element).find('.build-weapon-name').first().text().trim();
      const artifactName = $(element).find('.build-weapon-name').last().text().trim();

      const stats = [];
      $(element).find('.build-stats-item').each((j, statElement) => {
...
async function team() {
  try {
    ...
    $('.teams-item').each((index, element) => {
      ...
      $(element).find('.character-portrait').each((i, el) => {
        ...
        characters.push({ name: characterName, icon: characterIcon, type: characterType });
      });
...
async function tier() {
    try {
        ...
        const version = $('.tierlist-disclaimer:contains("Version")').text().replace('Version ', '').trim();
        ...
        $('.dropzone-row .tierlist-portrait.lg').each((index, element) => {
            ...
            const rarity = characterElement.find('.tierlist-icon').attr('class').includes('rarity-5') ? 5 : 4;
            const elementt = characterElement.find('.tierlist-type').attr('alt').trim();
